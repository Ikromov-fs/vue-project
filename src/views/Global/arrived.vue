        ...
        <section class="product__lists-table">
            ...
            <appTable :body="product__data" :headers="headers">
                ...
                <template #body_action="{ item }">
                    <span v-if="item.status === 'new'" @click="editIncovice(item)"><i
                            class="mx-2 deleted-invoice  fa-solid fa-pen-to-square"></i></span>
                    <router-link :to="{ name: 'input-invoice', params: { id: item.id } }">
                        <span><i class="mx-2  bi bi-eye-fill"></i></span>
                    </router-link>
                    <span v-if="item.status === 'new'" @click="deleteInovice(item)"><i
                            class="mx-2 deleted-invoice fa-solid fa-trash-can"></i></span>
                </template>
            </appTable>
...
const headers = ref([
    { title: '№', value: 'index' },
    { title: 'Supplier', value: 'address' },
    { title: 'Total sum', value: 'total_sum' },
    { title: 'Status', value: 'status' },
    { title: 'Description', value: 'description' },
    { title: 'Address', value: 'action' },
])
const form = ref({
    description: '',
})
